...
i0 = 0;         % current initial condition (A)
v  = 5;         % voltage source (V)
r  = 750;       % Resistance in ohms
l  = 10;        % Inductance in henry
...
for m = 1 : length(t)
    if (m == 1)
       i(m) = i0;                   % i= initial condition
    else
       i(m) = (v * dt + l * i(m-1))/(l + r * dt);   % deduced dynamics
    end 
end
